// seed.js
// Script para popular o banco de dados com filmes iniciais
...
// Modelo Filme (mesmo do index.js)
...
async function seed() {
  try {
    ...
    await sequelize.sync({ force: false }); // Use force: true para recriar as tabelas
    console.log('Tabelas sincronizadas.');
    
    // Verifica se já existem filmes
    const count = await Filme.count();
    if (count > 0) {
      console.log(`Já existem ${count} filmes no banco. Pulando seed.`);
      console.log('Se deseja recarregar os dados, delete o arquivo database.sqlite e execute novamente.');
      process.exit(0);
    }
    
    // Insere os filmes
    console.log('Inserindo filmes...');
    for (const filmeData of filmesIniciais) {
      await Filme.create(filmeData);
      console.log(`✓ ${filmeData.Title}`);
    }
    
    console.log(`\n✓ ${filmesIniciais.length} filmes inseridos com sucesso!`);
    console.log('\nEstatísticas:');
    
    const emCartaz = await Filme.count({ where: { ComingSoon: false } });
    const lancamentos = await Filme.count({ where: { ComingSoon: true } });
    const movies = await Filme.count({ where: { Type: 'movie' } });
    const series = await Filme.count({ where: { Type: 'series' } });
    
    console.log(`- Filmes em cartaz: ${emCartaz}`);
